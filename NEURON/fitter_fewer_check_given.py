#fitter_fewer_check.py: Runs the fitted models and records the time courses in
#high resolution, needed for Fig. 9A--C.
...
import random
import sys
import protocols_many_78withoutCK_1withCK as protocols_many
import scipy.io
...
def func_to_optimize(parameters,saveFig="",deleteFiles=True,rankID=0):
  ...
  if len(saveFig) > 0:
    ...
    f.text(0.01,0.01,str(parameters),fontsize=2)
...
def checkpopulation(population, columns, gen, gensdonealready):
  ...
  print 'Generation '+str(gen)+' done, saving to fewer'+str(imeas)+'_seed'+str(myseed)+'_N'+str(N_samples)+'_tmp'+str(gensdonealready+gen)+'.sav'
  file=open('fewer'+str(imeas)+'_seed'+str(myseed)+'_N'+str(N_samples)+'_tmp'+str(gensdonealready+gen)+'.sav', 'w')
...
random.seed(12892+imeas*1000000+myseed+istart)
if imeas == 1:
  filename = 'fewerCK1imeas'+str(imeas)+'_seed'+str(myseed)+'_N'+str(N_samples)
else:
  filename = 'fewer'+str(imeas)+'_seed'+str(myseed)+'_N'+str(N_samples)
...
A=scipy.io.loadmat('fits_goodparams.mat')
if A['params'].shape[1] == 1:
  A['params'] = A['params'].T
goodparams = A['params'][0][imeas]
