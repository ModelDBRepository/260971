#fitter_fewer.py: Runs the fitting of the model to cortical LTP/LTD data (Fig. 9)
#
#This is a very heavy operation where parallelization is necessary. Personally,
#I used 64 or 128 cores run in parallel as follows:
#mpirun -np 64 python fitter_fewer.py $DATA_ID $SEED 1000
#where DATA_ID is the number of the data set, from 0 to (and including) 10,
#and SEED is a random number seed needed for initializing the particles (and
#the evolution that is done after each set of objective function evaluations).
#
#All cores should be comparable in efficiency: if a simulation task takes
#too long to finish, a maximum error is given to the particle and the
#remaining simulations are skipped. Still, given the 64-128 cores, the
#optimisation took me 2-6 days for each DATA_ID - some data sets were much
#heavier to run than others.
#
...
import random
import sys
import protocols_many_78withoutCK as protocols_many
import scipy.io
...
def checkpopulation(population, columns, gen):
  ...
  print 'Generation '+str(gen)+' done, saving to fitfiles/fewer'+str(imeas)+'_seed'+str(myseed)+'_N'+str(N_samples)+'_tmp'+str(gen)+'.sav'
  file=open('fitfiles/fewer'+str(imeas)+'_seed'+str(myseed)+'_N'+str(N_samples)+'_tmp'+str(gen)+'.sav', 'w')
...
random.seed(37485+imeas*1000000+myseed)
...
myemoo.get_objectives_error = lambda params: func_to_optimize(params,'',True,myemoo.comm.rank)
myemoo.checkpopulation = lambda population, columns, gen: checkpopulation(population, columns, gen)

print 'Starting fitting, myrank = '+str(myemoo.comm.rank)+', N_samples = '+str(N_samples)+', Ngenerations = '+str(N_generations)+', myseed = '+str(myseed)
myemoo.evolution(generations = N_generations)

if myemoo.master_mode:
  params_all = myemoo.getpopulation_unnormed()

  picklelist = [N_samples, C_samples, N_generations, params_all.tolist()]
  file=open('fitfiles/fewer'+str(imeas)+'_seed'+str(myseed)+'_N'+str(N_samples)+'_wholepop.sav', 'w')
  pickle.dump(picklelist,file)
  file.close()
