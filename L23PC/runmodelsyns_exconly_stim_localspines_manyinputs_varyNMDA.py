...
headLen = 0.4 #These values of headlen and headdiam make up 0.5um3
headDiam = 0.4 #These values of headlen and headdiam make up 0.5um3
dodraw = 1
dendtree = 'apic'
...
if len(sys.argv) > 3:
  stimN = int(float(sys.argv[3]))
if len(sys.argv) > 4:
  Nsyn = int(sys.argv[4])
if len(sys.argv) > 5:
  Ninputs = int(sys.argv[5])
if len(sys.argv) > 6:
  dendtree = sys.argv[6]
if len(sys.argv) > 7:
  spinelocations = sys.argv[7]
if len(sys.argv) > 8:
  rateE = float(sys.argv[8])
if len(sys.argv) > 9:
  Npulses = int(float(sys.argv[9]))
if len(sys.argv) > 10:
  ISIpairing = float(sys.argv[10])
if len(sys.argv) > 11:
  dtpulses = float(sys.argv[11])
if len(sys.argv) > 12:
  pulseamp = float(sys.argv[12])
if len(sys.argv) > 13:
  neckLen = float(sys.argv[13])
if len(sys.argv) > 14:
  neckDiam = float(sys.argv[14])
if len(sys.argv) > 15:
  Econ = float(sys.argv[15])
if len(sys.argv) > 16:
  weight_factor_NMDA = float(sys.argv[16])
if len(sys.argv) > 17:
  rdSeed = int(float(sys.argv[17]))
if len(sys.argv) > 18:
  dodraw = int(float(sys.argv[18]))
...
{load_file("L23_PC_cADpyr229_"""+str(icell)+"""/constants.hoc")}
...
""")
  for iinput in range(0,Ninputs):
    h("""
{spineHead["""+str(isyn)+"""](0) synlist.append(new ProbAMPANMDA2(0.5))}
...
scipy.io.savemat("noisy_icell"+str(icell)+"_n"+str(stimN)+"_"+str(stimfreq)+"_neckLen"+str(neckLen)+"_neckDiam"+str(neckDiam)+"_Nsyn"+str(Nsyn)+"_Ninputs"+str(Ninputs)+dendtree+spinelocations+"_Econ"+str(Econ)+'_wNMDA'+str(weight_factor_NMDA)+"_rateE"+str(rateE)+"_Npulses"+str(Npulses)+"_ISI"+str(ISIpairing)+"_dtpulses"+str(dtpulses)+"_pulseamp"+str(pulseamp)+"_seed"+str(rdSeed)+".mat",
...
if dodraw:
  ...
  f.savefig("vsoma_icell"+str(icell)+"_n"+str(stimN)+"_"+str(stimfreq)+"_neckLen"+str(neckLen)+"_neckDiam"+str(neckDiam)+"_Nsyn"+str(Nsyn)+"_Ninputs"+str(Ninputs)+dendtree+spinelocations+"_Econ"+str(Econ)+'_wNMDA'+str(weight_factor_NMDA)+"_rateE"+str(rateE)+"_Npulses"+str(Npulses)+"_ISI"+str(ISIpairing)+"_dtpulses"+str(dtpulses)+"_pulseamp"+str(pulseamp)+"_seed"+str(rdSeed)+".eps")
