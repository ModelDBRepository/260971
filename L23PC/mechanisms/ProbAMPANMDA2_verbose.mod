...
NEURON {
        ...
        POINT_PROCESS ProbAMPANMDA2_verbose  
        ...
        if (Fac > 0) {
		   ...
		   if (erand() < Pr){
		    ...
		    A_NMDA = A_NMDA + weight*weight_factor_NMDA*factor_NMDA
                    ...
                    printf( "Release! time %g: vals %g %g %g %g, Pv = %g, Pr = %g\n", t, A_AMPA, weight, factor_AMPA, weight_factor_NMDA, Pv, Pr )
                                  
                  }
                  else {
                    printf("Failure! time %g: Pv = %g, Pr = %g\n", t, Pv, Pr )
